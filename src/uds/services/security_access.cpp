...
SecurityAccessService::SecurityAccessService()
    : m_level_count(0), m_active_level(0)
{
...
{
    if (m_level_count >= kMaxSecurityLevels)
        return false;
    if ((level.level & 1) == 0)
        return false; // level must be odd (requestSeed subfunction)
...
{
    uint8_t req_seed = (level & 1) ? level : (level - 1);
    for (size_t i = 0; i < m_level_count; ++i) {
        if (m_levels[i].cfg.level == req_seed)
            return &m_levels[i];
    }
    return nullptr;
}
...
{
    for (size_t i = 0; i < m_level_count; ++i) {
        if (m_levels[i].cfg.level == level)
            return m_levels[i].unlocked;
    }
    return false;
}
    ...
    if (is_seed) {
        ...
        if (!valid) {
            ...
            if (lvl->fail_count >= kMaxAttempts) {
                lvl->locked_until = kDelayAfterFail; // TODO: use real tick
